...
exports.getTransactions = async (req, res) => {
  try {
    ...
    let data = await transaction.findAll({
      ...
      attributes: {
        exclude: ['updatedAt', 'idBuyer', 'idSeller', 'idProduct', 'createdAt'],
      },
      include: [
        {
          model: product,
          as: 'product',
          attributes: {
            exclude: ['createdAt', 'updatedAt', 'idUser', 'stock', 'price', 'desc'],
          },
        },
        {
          model: user,
...
exports.addTransaction = async (req, res) => {
  try {
    ...
    // Parameter terkait data yang akan dikirim ke midtrans
    let parameter = {
      transaction_details: {
        order_id: newData.id,
...
// Handle update product stock/stock
const updateProduct = async (orderId) => {
  ...
  const stock = productData.stock - 1;
  await product.update({ stock }, { where: { id: productData.id } });
};
